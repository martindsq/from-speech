...
class CoWaverDualRoute(TrainableModule):
    def __init__(self, latent_dim: int = 256, hidden_size: int = 256, seq_len: int = 49, mel_bins: int = 40, width_steps: int = 24, height_bands: int = 4, num_tasks: int = 2, adapter: str = "convolutional", decoder: str = "convolutional"):
        ...
        super().__init__(name=f"cowaver_dual_route_lt{latent_dim}_hs{hidden_size}_sl{seq_len}_mb{mel_bins}_ws{width_steps}{height_suffix}{adapter_suffix}{decoder_suffix}")
    ...
    def _resolve_task_ids(self, task_ids, device: torch.device) -> Tensor:
        if task_ids is None:
            raise ValueError("CoWaverDualRoute requires task_ids from the dataset.")
