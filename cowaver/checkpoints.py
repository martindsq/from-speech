...
def fase_mas_alta(net: TrainableModule) -> int:
    prefix = f"{net.name}_"
    suffix = "_phase.pt"
    phases = []

    for path in CHECKPOINTS_ROOT.glob(f"{net.name}_*_phase.pt"):
        name = path.name
    ...
    if not phases:
        raise FileNotFoundError(f"No checkpoints found for '{net.name}' in {CHECKPOINTS_ROOT}")

    return max(phases)

def ruta_al_checkpoint(net: TrainableModule, phase: int):
    ordinal = lambda n: "%d%s" % (n,"tsnrhtdd"[(n//10%10!=1)*(n%10<4)*n%10::4])
    return CHECKPOINTS_ROOT / f"{net.name}_{ordinal(phase)}_phase.pt"

def guardar_checkpoint(net: TrainableModule, train_history: TrainHistory, phase: int = 1):
    path = ruta_al_checkpoint(net, phase)
...
def imprimir_encabezado(net: TrainableModule, phase: int = 1):
    ...
    line = "═" * 80
    print(f"\n{line}")
    print(f"Fase {phase} | checkpoint: {path}")
    print(f"{line}")

def cargar_checkpoint(net: TrainableModule, device: device = device("cpu"), phase: int | None = None, silent: bool = False):
    if phase is None:
